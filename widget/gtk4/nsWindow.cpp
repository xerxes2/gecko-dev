...
static GtkWidget* get_gtk_widget_for_gdk_window(GdkSurface* window);
...
static void toplevel_window_size_allocate_cb(GdkSurface* aSurface,
...
static gboolean motion_notify_event_cb(GtkEventControllerMotion* controller,
                                      gdouble pos_x, gdouble pos_y);
MOZ_CAN_RUN_SCRIPT static gboolean button_press_event_cb(GtkGestureClick* gesture,
                                 gint n_press, double pos_x, gdouble pos_y);
...
static void display_composited_changed_cb(GdkDisplay* display, gpointer user_data);
...
static void scale_changed_cb(GtkWidget* widget, GParamSpec* aPSpec,
                             gpointer aPointer);
static gboolean touch_event_cb(GtkGesture* gesture,
...
static gboolean drag_motion_event_cb(GtkWidget* aWidget,
                                     GdkDrag* aDragContext, gint aX,
                                     gint aY, guint aTime, gpointer aData);
static void drag_leave_event_cb(GtkWidget* aWidget,
                                GdkDrag* aDragContext, guint aTime,
                                gpointer aData);
...
MOZ_RUNINIT static GdkCrossingEvent* sStoredLeaveNotifyEvent;
...
nsWindow::nsWindow()
    : mWindowVisibilityMutex("nsWindow::mWindowVisibilityMutex"),
      ...
      mIsDragPopup(false),
      mCompositedScreen(ScreenHelperGTK::IsComposited()),
...
void nsWindow::DispatchResized() {
  ...
  LayoutDeviceIntSize toplevelBounds(gtk_widget_get_width(mShell), gtk_widget_get_height(mShell));
  if (toplevelBounds.width == mBounds.width && toplevelBounds.height == mBounds.height) {
    return;
  }
  mBounds.width = toplevelBounds.width;
  mBounds.height = toplevelBounds.height;
...
//printf("nsWindow::DispatchResized");
  // Notify the GtkCompositorWidget of a ClientSizeChange
  if (mCompositorWidgetDelegate) {
    mCompositorWidgetDelegate->NotifyClientSizeChanged(toplevelBounds);
  }
...
void nsWindow::Destroy() {
  ...
  */
  ...
  if (sStoredLeaveNotifyEvent) {
    nsWindow* window =
        get_window_for_gdk_window(gdk_event_get_surface(GDK_EVENT(sStoredLeaveNotifyEvent)));
    if (window == this) {
...
static void InitPenEvent(WidgetMouseEvent& aGeckoEvent, GdkEvent* aEvent) {
  ...
  GdkDevice* device = gdk_event_get_device(aEvent);
...
void nsWindow::WaylandPopupHierarchyCalculatePositions() {
  ...
  while (popup) {
    ...
    if (popup->WaylandPopupIsFirst()) {
      ...
      GdkRectangle parent = popup->WaylandGetParentPosition();
...
GdkRectangle nsWindow::WaylandGetParentPosition() {
  GdkRectangle topLeft = {0, 0};
...
static void NativeMoveResizeCallback(GdkSurface* window,
...
void nsWindow::NativeMoveResizeWaylandPopupCallback(
  ...
  const LayoutDeviceIntRect newBounds = [&] {
    ...
    GdkRectangle parent = WaylandGetParentPosition();
...
void nsWindow::WaylandPopupSetDirectPosition() {
  GdkRectangle topLeft = DevicePixelsToGdkPointRoundDown(mBounds.TopLeft());
  ...
  GdkSurface* gdkWindow = window->GetToplevelSurface();
  if (!gdkWindow) {
    return;
  }

  //int parentWidth = gdk_surface_get_width(gdkWindow);
...
bool nsWindow::WaylandPopupFitsToplevelWindow(bool aMove) {
  ...
  GdkSurface* toplevelGdkWindow = gtk_native_get_surface(gtk_widget_get_native(GTK_WIDGET(parent)));
  if (!toplevelGdkWindow) {
    NS_WARNING("Toplevel widget without GdkWindow?");
    return false;
  }

  int parentWidth = gdk_surface_get_width(toplevelGdkWindow);
  int parentHeight = gdk_surface_get_height(toplevelGdkWindow);
  LOG("  parent size %d x %d", parentWidth, parentHeight);

  GdkRectangle topLeft = aMove ? mPopupPosition
...
bool nsWindow::WaylandPopupAnchorAdjustForParentPopup(
    GdkRectangle* aPopupAnchor, GdkRectangle* aOffset) {
  ...
  GdkSurface* window = gtk_native_get_surface(gtk_widget_get_native(GTK_WIDGET(parentGtkWindow)));
  if (!window) {
    NS_WARNING("Popup parrent is not mapped!");
    return false;
  }

  GdkRectangle parentWindowRect = {0, 0, gdk_surface_get_width(window),
...
bool nsWindow::WaylandPopupCheckAndGetAnchor(GdkRectangle* aPopupAnchor,
  ...
  GdkSurface* gdkWindow = GetToplevelSurface();
  nsMenuPopupFrame* popupFrame = GetMenuPopupFrame(GetFrame());
...
void nsWindow::WaylandPopupMovePlain(int aX, int aY) {
  ...
  /*if (!gtk_widget_get_mapped(mShell)) {
    if (GdkSurface* window = GetToplevelGdkWindow()) {
...
void nsWindow::WaylandPopupMoveImpl() {
  ...
  static auto sGdkWindowMoveToRect = (void (*)(
      GdkSurface*, const GdkRectangle*, GdkGravity, GdkGravity, GdkAnchorHints,
...
void nsWindow::SetSizeMode(nsSizeMode aMode) {
  ...
  switch (aMode) {
    ...
    case nsSizeMode_Minimized:
      LOG("    set minimized");
      gtk_window_minimize(GTK_WINDOW(mShell));
      break;
    ...
    case nsSizeMode_Normal:
      ...
      if (SizeModeMightBe(nsSizeMode_Minimized)) {
        gtk_window_unminimize(GTK_WINDOW(mShell));
...
static bool WorkspaceManagementDisabled(GdkDisplay* display) {
...
void nsWindow::SetUserTimeAndStartupTokenForActivatedWindow() {
  ...
  if (!mWindowActivationTokenFromEnv.IsEmpty()) {
    ...
    // that triggered it.
    gtk_widget_grab_focus(GTK_WIDGET(mShell));
  }
...
#else
  GdkSeat* seat = gdk_display_get_default_seat(gdkDisplay);
  GdkDevice* pointer = gdk_seat_get_pointer(seat);
  guint32 timestamp = gdk_device_get_timestamp(pointer);
#endif
...
void nsWindow::FocusWaylandWindow(const char* aTokenID) {
  ...
  wl_surface* surface =
      mShellSurface ? gdk_wayland_surface_get_wl_surface(mShellSurface) : nullptr;
  if (!surface) {
...
void nsWindow::SetFocus(Raise aRaise, mozilla::dom::CallerType aCallerType) {
  ...
  // set properly.
  //GtkWidget* toplevelWidget = GTK_WIDGET(gtk_widget_get_root(GTK_WIDGET(mContainer)));
  GtkWidget* toplevelWidget = mShell;
  ...
  if (StaticPrefs::mozilla_widget_raise_on_setfocus_AtStartup() &&
    ...
    if (gtk_widget_get_visible(mShell)) {
      ...
      gtk_widget_set_visible(toplevelWidget, true);
...
static GdkCursor* GetCursorForImage(const nsIWidget::Cursor& aCursor,
  ...
  auto CleanupSurface =
      MakeScopeExit([&]() { g_object_unref(texture); });

  return gdk_cursor_new_from_texture(texture,
                                     aCursor.mHotspotX, aCursor.mHotspotY, nullptr);
}
...
void nsWindow::SetCursor(const Cursor& aCursor) {
  ...
  gdk_surface_set_cursor(mGdkWindow, nonImageCursor);
  if (imageCursor) {
    gdk_surface_set_cursor(mGdkWindow, imageCursor);
  }
}
...
void nsWindow::SetIcon(const nsAString& aIconSpec) {
  ...
  gint* iconSizes = gtk_icon_theme_get_icon_sizes(
    gtk_icon_theme_get_for_display(gdk_display_get_default()), iconName.get());
    ...
    for (uint32_t i = 0; i < std::size(extensions); i++) {
      ...
      if (iconFile) {
        ...
        gtk_icon_theme_add_resource_path(
          gtk_icon_theme_get_for_display(gdk_display_get_default()), path.get());
          //g_object_unref(icon);
          foundIcon = true;
        //}
      }
...
gboolean nsWindow::OnConfigureEvent(GtkWidget* aWidget,
...
#ifdef MOZ_LOGGING
  //int scale = mGdkWindow ? gdk_surface_get_scale_factor(mGdkWindow) : -1;
  ...
  if (mGdkWindow && IsTopLevelWidget()) {
    if (mCeiledScaleFactor != gdk_surface_get_scale_factor(mGdkWindow)) {
      LOG("  scale factor changed to %d,return early",
          gdk_surface_get_scale_factor(mGdkWindow));
      return FALSE;
    }
  }
  ...
  if (mGdkWindow &&
      gtk_window_get_transient_for(GTK_WINDOW(aWidget))) {
...
void nsWindow::OnEnterNotifyEvent(GdkEvent* aEvent) {
  ...
  event.mRefPoint = GdkEventCoordsToDevicePixels(x, y);
  event.AssignEventTime(GetWidgetEventTime(gdk_event_get_time((GDK_EVENT(aEvent)))));
...
static bool IsBogusLeaveNotifyEvent(GdkSurface* aWindow,
  ...
  GdkSurface* winAtPt =
      gdk_device_get_surface_at_position(pointer, nullptr, nullptr);
  if (!winAtPt) {
    return false;
  }
  // We're still in the same top level window, ignore this leave notify event.
  //GdkSurface* topLevelAtPt = gdk_window_get_toplevel(winAtPt);
  //GdkSurface* topLevelWidget = gdk_window_get_toplevel(aWindow);
...
Maybe<GdkSurfaceEdge> nsWindow::CheckResizerEdge(
  ...
  if (topDist <= resizerHeight && mResizableEdges.Top()) {
    if (rightDist <= resizerWidth && mResizableEdges.Right()) {
      return Some(GDK_SURFACE_EDGE_NORTH_EAST);
    }
    if (leftDist <= resizerWidth && mResizableEdges.Left()) {
      return Some(GDK_SURFACE_EDGE_NORTH_WEST);
    }
    return waylandLimitedResize ? Nothing() : Some(GDK_SURFACE_EDGE_NORTH);
  }

  if (bottomDist <= resizerHeight && mResizableEdges.Bottom()) {
    if (rightDist <= resizerWidth && mResizableEdges.Right()) {
      return Some(GDK_SURFACE_EDGE_SOUTH_EAST);
    }
    if (leftDist <= resizerWidth && mResizableEdges.Left()) {
      return Some(GDK_SURFACE_EDGE_SOUTH_WEST);
    }
    return Some(GDK_SURFACE_EDGE_SOUTH);
  }
  ...
  if (leftDist <= resizerHeight && mResizableEdges.Left()) {
    ...
    return waylandLimitedResize ? Nothing() : Some(GDK_SURFACE_EDGE_WEST);
  }
  ...
  if (rightDist <= resizerHeight && mResizableEdges.Right()) {
    ...
    return Some(GDK_SURFACE_EDGE_EAST);
  }
  return Nothing();
}
...
static LayoutDeviceIntPoint GetRefPoint(nsWindow* aWindow, Event* aEvent) {
  double x, y;
  if (gdk_event_get_surface(aEvent) == aWindow->GetGdkWindow()) {
    // we are the window that the event happened on so no need for expensive
    // WidgetToScreenOffset
    gdk_event_get_position(aEvent, &x, &y);
    return aWindow->GdkEventCoordsToDevicePixels(x, y);
  }
  return aWindow->GdkEventCoordsToDevicePixels(x, y);
...
void nsWindow::OnMotionNotifyEvent(GdkEvent* aEvent) {
  ...
  if (mWindowShouldStartDragging) {
    mWindowShouldStartDragging = false;
    GdkSurface* dragWindow = nullptr;

    // find the top-level window
    if (mGdkWindow) {
      //dragWindow = gdk_window_get_toplevel(mGdkWindow);
      dragWindow = gtk_native_get_surface(gtk_widget_get_native(mShell));
  ...
  event.AssignEventTime(GetWidgetEventTime(gdk_event_get_time((GDK_EVENT(aEvent)))));
  KeymapWrapper::InitInputEvent(event, gdk_event_get_modifier_state((GDK_EVENT(aEvent))));
...
void nsWindow::DispatchMissedButtonReleases(GdkEvent* aGdkEvent) {
  ...
  gButtonState = gdk_event_get_modifier_state(GDK_EVENT(aGdkEvent));
...
void nsWindow::InitButtonEvent(WidgetMouseEvent& aEvent,
  ...
  guint modifierState = gdk_event_get_modifier_state(GDK_EVENT(aGdkEvent));
  ...
  // the button state.
  guint buttonMask = 0;
  switch (gdk_button_event_get_button(GDK_EVENT(aGdkEvent))) {
    case 1:
  ...
  if (gdk_event_get_event_type(GDK_EVENT(aGdkEvent)) == GDK_BUTTON_RELEASE) {
  ...
  aEvent.AssignEventTime(GetWidgetEventTime(gdk_event_get_time(GDK_EVENT(aGdkEvent))));
  /*
  switch (gdk_event_get_event_type(GDK_EVENT(aGdkEvent))) {
...
void nsWindow::OnButtonPressEvent(GdkEvent* aGdkEvent) {
  ...
  */
  ...
  if (auto edge = CheckResizerEdge(refPoint)) {
    ...
    double x, y;
    gdk_event_get_position(GDK_EVENT(aGdkEvent), &x, &y);
    if (mAspectRatio != 0.0f && GdkIsWaylandDisplay()) {
      mLastResizePoint = LayoutDeviceIntPoint::Floor(x, y);
      switch (*edge) {
        case GDK_SURFACE_EDGE_SOUTH:
  ...
  gdouble pressure = 0;
  gdk_event_get_axis((GdkEvent*)aGdkEvent, GDK_AXIS_PRESSURE, &pressure);
  mLastMotionPressure = pressure;
  ...
  gButtonState |= ButtonMaskFromGDKButton(gdk_button_event_get_button(GDK_EVENT(aGdkEvent)));

  WidgetMouseEvent event(true, eMouseDown, this, WidgetMouseEvent::eReal);
  event.mButton = domButton;
  InitButtonEvent(event, aGdkEvent, refPoint);
  event.mPressure = mLastMotionPressure;

  InitPenEvent(event, (GdkEvent*)aGdkEvent);
  ...
  if (!defaultPrevented && mDraggableRegion.Contains(refPoint)) {
    if (domButton == MouseButton::ePrimary) {
      ...
      DoTitlebarAction(nsXPLookAndFeel::TitlebarEvent::Middle_Click, aGdkEvent);
    }
  }
  ...
  if (!StaticPrefs::ui_context_menus_after_mouseup() &&
    ...
    DispatchContextMenuEventFromMouseEvent(domButton, aGdkEvent, refPoint);
  }
}
...
void nsWindow::OnButtonReleaseEvent(GdkEvent* aGdkEvent) {
  ...
  if (!defaultPrevented && mDrawInTitlebar &&
    ...
    DoTitlebarAction(nsXPLookAndFeel::TitlebarEvent::Double_Click, aGdkEvent);
  }
  mLastMotionPressure = pressure;
  ...
  if (mAlwaysOnTop && gdk_button_event_get_button(GDK_EVENT(aGdkEvent)) == 3) {
    TryToShowNativeWindowMenu(aGdkEvent);
  }
}
...
void nsWindow::OnScrollEvent(GdkEvent* aGdkEvent) {
  ...
  if (gdk_event_get_time(GDK_EVENT(aGdkEvent)) != GDK_CURRENT_TIME &&
      mLastSmoothScrollEventTime == gdk_event_get_time(GDK_EVENT(aGdkEvent))) {
    return;
  }
  ...
  gdk_scroll_event_get_deltas(GDK_EVENT(aGdkEvent), &delta_x, &delta_y);

  // check for duplicate legacy scroll event, see GNOME bug 726878
  if (gdk_scroll_event_get_direction(GDK_EVENT(aGdkEvent)) != GDK_SCROLL_SMOOTH &&
      mLastScrollEventTime == gdk_event_get_time(GDK_EVENT(aGdkEvent))) {
    LOG("[%d] duplicate legacy scroll event %d\n", gdk_event_get_time(GDK_EVENT(aGdkEvent)),
        gdk_scroll_event_get_direction(GDK_EVENT(aGdkEvent)));
    return;
  }
  WidgetWheelEvent wheelEvent(true, eWheel, this);
  wheelEvent.mDeltaMode = dom::WheelEvent_Binding::DOM_DELTA_LINE;
  switch (gdk_scroll_event_get_direction(GDK_EVENT(aGdkEvent))) {
    case GDK_SCROLL_SMOOTH: {
      ...
      GdkDevice* device = gdk_event_get_device(GDK_EVENT(aGdkEvent));
      ...
      if (source == GDK_SOURCE_TOUCHSCREEN || source == GDK_SOURCE_TOUCHPAD ||
        ...
        if (StaticPrefs::apz_gtk_pangesture_enabled() &&
          ...
          LOG("[%d] pan smooth event dx=%f dy=%f inprogress=%d\n", gdk_event_get_time(GDK_EVENT(aGdkEvent)),
              delta_x, delta_y, mPanInProgress);
          auto eventType = PanGestureInput::PANGESTURE_PAN;
          if (sGdkEventIsScrollStopEvent((GdkEvent*)aGdkEvent)) {
          ...
          ScreenPoint deltas(float(delta_x * multiplier),
                             float(delta_y * multiplier));

          LayoutDeviceIntPoint touchPoint = GetRefPoint(this, aGdkEvent);
          PanGestureInput panEvent(
              eventType, GetEventTimeStamp(gdk_event_get_time(GDK_EVENT(aGdkEvent))),
              ScreenPoint(touchPoint.x, touchPoint.y), deltas,
              KeymapWrapper::ComputeKeyModifiers(gdk_event_get_modifier_state(GDK_EVENT(aGdkEvent))));
      ...
      wheelEvent.mDeltaX = delta_x * 3;
      wheelEvent.mDeltaY = delta_y * 3;
      wheelEvent.mWheelTicksX = delta_x;
      wheelEvent.mWheelTicksY = delta_y;
  ...
  wheelEvent.mRefPoint = GetRefPoint(this, aGdkEvent);

  KeymapWrapper::InitInputEvent(wheelEvent, gdk_event_get_modifier_state(GDK_EVENT(aGdkEvent)));

  wheelEvent.AssignEventTime(GetWidgetEventTime(gdk_event_get_time(GDK_EVENT(aGdkEvent))));
...
void nsWindow::RefreshScale(bool aRefreshScreen) {
  ...
  LOG("nsWindow::RefreshScale() GdkWindow scale %d refresh %d",
      gdk_surface_get_scale_factor(mGdkWindow), aRefreshScreen);

#ifdef MOZ_WAYLAND
  if (GdkIsWaylandDisplay()) {
    WaylandSurfaceLock lock(mSurface);
    mSurface->SetCeiledScaleLocked(lock,
                                   gdk_surface_get_scale_factor(GetToplevelSurface()));
  }
#endif

  MOZ_DIAGNOSTIC_ASSERT(mIsMapped && GetToplevelSurface());
  int ceiledScale = gdk_surface_get_scale_factor(GetToplevelSurface());
...
gboolean nsWindow::OnTouchpadPinchEvent(GdkEvent* aGdkEvent) {
  ...
  if (gdk_touchpad_event_get_n_fingers(GDK_EVENT(aGdkEvent)) > 2 &&
  ...
  switch (gdk_touchpad_event_get_gesture_phase(GDK_EVENT(aGdkEvent))) {
    case GDK_TOUCHPAD_GESTURE_PHASE_BEGIN:
      pinchGestureType = PinchGestureInput::PINCHGESTURE_START;
      currentSpan = gdk_touchpad_event_get_pinch_scale(GDK_EVENT(aGdkEvent));
      mCurrentTouchpadFocus = ViewAs<ScreenPixel>(
          GetRefPoint(this, aGdkEvent),
    ...
    case GDK_TOUCHPAD_GESTURE_PHASE_UPDATE:
      ...
      double dx, dy;
      gdk_scroll_event_get_deltas(GDK_EVENT(aGdkEvent), &dx, &dy);
      mCurrentTouchpadFocus += ScreenPoint(dx, dy);
      if (gdk_touchpad_event_get_pinch_scale(GDK_EVENT(aGdkEvent)) == mLastPinchEventSpan) {
        return FALSE;
      }
      currentSpan = gdk_touchpad_event_get_pinch_scale(GDK_EVENT(aGdkEvent));
  ...
  PinchGestureInput event(
      ...
      GetEventTimeStamp(gdk_event_get_time(GDK_EVENT(aGdkEvent))), ExternalPoint(0, 0),
      mCurrentTouchpadFocus,
      100.0 * ((gdk_touchpad_event_get_gesture_phase(GDK_EVENT(aGdkEvent)) == GDK_TOUCHPAD_GESTURE_PHASE_END)
                   ? ScreenCoord(1.f)
                   : currentSpan),
      100.0 * ((gdk_touchpad_event_get_gesture_phase(GDK_EVENT(aGdkEvent)) == GDK_TOUCHPAD_GESTURE_PHASE_END)
                   ? ScreenCoord(1.f)
  ...
  mLastPinchEventSpan = gdk_touchpad_event_get_pinch_scale(GDK_EVENT(aGdkEvent));
...
gboolean nsWindow::OnTouchEvent(GdkEvent* aGdkEvent) {
  double pos_x, pos_y;
  gdk_event_get_position(GDK_EVENT(aGdkEvent), &pos_x, &pos_y);
  guint32 time = gdk_event_get_time(GDK_EVENT(aGdkEvent));
  GdkEventType type = gdk_event_get_event_type(GDK_EVENT(aGdkEvent));
  GdkModifierType state = gdk_event_get_modifier_state(GDK_EVENT(aGdkEvent));
  ...
  if (!mHandleTouchEvent) {
    ...
    //  return targetWindow->OnTouchEvent(aGdkEvent);
    //}
  ...
  switch (type) {
    ...
    case GDK_TOUCH_UPDATE:
      ...
      if (mWindowShouldStartDragging) {
        mWindowShouldStartDragging = false;
        if (mGdkWindow) {
          GdkSurface* gdk_window = GetSurfaceFromWidget(mShell);
  ...
  const LayoutDeviceIntPoint touchPoint = GetRefPoint(this, aGdkEvent);
  ...
  KeymapWrapper::InitInputEvent(event, state);
  /*
...
static void GtkWidgetDisableUpdates(GtkWidget* aWidget) {
  ...
  GdkSurface* window = gtk_native_get_surface(GTK_NATIVE(gtk_widget_get_native(aWidget)));
  if (!window) {
    return;
  }
...
void nsWindow::EnsureGdkSurface() {
  MOZ_DIAGNOSTIC_ASSERT(mIsMapped);
  if (!mShellSurface) {
    mShellSurface = GetToplevelSurface();
    mGdkWindow = GetToplevelSurface();
    g_object_set_data(G_OBJECT(mShellSurface), "nsWindow", this);
  }
}
  ...
  auto startCompositing = [self = RefPtr{this}, this]() -> void {
    ...
    if (mIsDestroyed || !mIsMapped) {
      LOG("  quit, mIsDestroyed = %d mIsMapped = %d", !!mIsDestroyed,
          !!mIsMapped);
      printf("  quit, mIsDestroyed = %d mIsMapped = %d", !!mIsDestroyed,
          !!mIsMapped);
      return;
    }
...
nsresult nsWindow::Create(nsIWidget* aParent, const LayoutDeviceIntRect& aRect,
  ...
  if (mIsPIPWindow) {
    ...
    if (mNoAutoHide) {
      ...
      gtk_widget_set_can_focus(mShell, FALSE);
#ifdef MOZ_X11
  ...
  g_object_set_data(G_OBJECT(GetToplevelSurface()), "nsWindow", this);
  ...
  //}
  //mSurface = new WaylandSurface(nullptr, gfx::IntSize(mLastSizeRequest.width, mLastSizeRequest.height));
  mSurface = new WaylandSurface(
        parentnsWindow ? MOZ_WL_SURFACE(parentnsWindow->GetMozContainer())
                       : nullptr,
        gfx::IntSize(mLastSizeRequest.width, mLastSizeRequest.height));
  mContainer = moz_container_new(this, mShell, mSurface);
#else
  ...
  //gtk_window_set_child(GTK_WINDOW(mShell), container);
  ...
  if (mIsChildWindow && parentnsWindow) {
    GdkSurface* window = GetToplevelSurface();
    GdkSurface* parentWindow = parentnsWindow->GetToplevelSurface();
  ...
  // Display events
  GdkDisplay* display = gtk_widget_get_display(mShell);
  if (!g_signal_handler_find(display, G_SIGNAL_MATCH_FUNC, 0, 0, nullptr,
                             FuncToGpointer(display_composited_changed_cb),
                             nullptr)) {
    g_signal_connect(display, "notify::composited",
                     G_CALLBACK(display_composited_changed_cb), nullptr);
  }
  // Main window events
  ...
  g_signal_connect(GetToplevelSurface(), "layout", G_CALLBACK(toplevel_window_size_allocate_cb), nullptr);
  //g_signal_connect(GDK_SURFACE(mShell), "layout", G_CALLBACK(toplevel_window_size_allocate_cb), nullptr);
  ...
  */
  ...
  g_signal_connect(mShell, "notify::scale-factor",
  ...
  //                 nullptr);
  //g_signal_connect(mContainer, "leave", G_CALLBACK(focus_out_event_cb),
  //                 nullptr);
  // Keyboard
  GtkEventController* controller_key = gtk_event_controller_key_new();
  gtk_widget_add_controller(mShell, controller_key);
  g_signal_connect(controller_key, "key-pressed",
                   G_CALLBACK(key_press_event_cb), nullptr);
  g_signal_connect(controller_key, "key-released",
  ...
  //                             ? mShell
  //                             : GTK_WIDGET(mContainer);
  //GtkWidget* eventWidget = mShell;
  // Pointer
  GtkEventController* controller_motion = gtk_event_controller_motion_new();
  gtk_widget_add_controller(mShell, controller_motion);
  g_signal_connect(controller_motion, "enter",
                   G_CALLBACK(enter_notify_event_cb), nullptr);
  g_signal_connect(controller_motion, "leave",
                   G_CALLBACK(leave_notify_event_cb), nullptr);
  g_signal_connect(controller_motion, "motion",
  ...
  // Scroll
  GtkEventController* controller_scroll = gtk_event_controller_scroll_new(
    GtkEventControllerScrollFlags(GTK_EVENT_CONTROLLER_SCROLL_VERTICAL |
                                  GTK_EVENT_CONTROLLER_SCROLL_HORIZONTAL));
  gtk_widget_add_controller(mShell, controller_scroll);
  g_signal_connect(controller_scroll, "scroll", G_CALLBACK(scroll_event_cb),
                   nullptr);
  // Swipe
  GtkGesture* gesture_swipe = gtk_gesture_swipe_new();
  gtk_widget_add_controller(mShell, GTK_EVENT_CONTROLLER(gesture_swipe));
  g_signal_connect(gesture_swipe, "begin", G_CALLBACK(touch_event_cb),
                   nullptr);
  // Generic events
  ...
  LOG("  mShell %p (window %p) mContainer %p mGdkWindow %p XID 0x%lx\n", mShell,
      GetToplevelSurface(), mContainer, mGdkWindow, GetX11Window());
...
GdkSurface* nsWindow::GetSurfaceFromWidget(GtkWidget* aWidget) {
  return gtk_native_get_surface(GTK_NATIVE(gtk_widget_get_native(aWidget)));
}
...
*/
static void toplevel_window_size_allocate_cb(GdkSurface* aSurface, gint width, gint height) {
  RefPtr<nsWindow> window = get_window_for_gdk_window(aSurface);
  if (!window) {
    return;
  }
...
*/
static void display_composited_changed_cb(GdkDisplay* display,
                                         gpointer user_data) {
...
static LayoutDeviceIntPoint GetWindowDropPosition(nsWindow* aWindow, int aX,
  ...
  if (aWindow->IsWaylandPopup()) {
    int tx = 0, ty = 0;
    tx = gdk_popup_get_position_x(GDK_POPUP(aWindow->GetToplevelSurface()));
    ty = gdk_popup_get_position_y(GDK_POPUP(aWindow->GetToplevelSurface()));
    aX += tx;
    aY += ty;
  }
...
/*
bool nsWindow::GetDragInfo(WidgetMouseEvent* aMouseEvent, GdkSurface** aWindow,
...
void nsWindow::SetCustomTitlebar(bool aState) {
  ...
  if (mGtkWindowDecoration == GTK_DECORATION_SYSTEM) {
    ...
    //GtkWidget* tmpWindow = gtk_window_new();
    //gtk_widget_realize(tmpWindow);
    //gtk_window_set_child(GTK_WINDOW(mShell), nullptr);
    //gtk_window_set_child(GTK_WINDOW(tmpWindow), GTK_WIDGET(mContainer));
    ...
    //GtkRequisition* nat_size = gtk_requisition_new();
    //gtk_widget_get_preferred_size(GTK_WIDGET(mShell), min_size, nat_size);
    //allocation.width = min_size->width;
    //gtk_widget_size_allocate(GTK_WIDGET(mShell), &allocation, -1);
    gtk_window_set_default_size(GTK_WINDOW(mShell), 900, 600);
    //gtk_widget_realize(GTK_WIDGET(mShell));
    //gtk_window_set_child(GTK_WINDOW(tmpWindow), nullptr);
    //gtk_window_set_child(GTK_WINDOW(mShell), GTK_WIDGET(mContainer));
    //gtk_requisition_free(min_size);
    //gtk_requisition_free(nat_size);
    ...
    //  mNeedsShow = true;
    //  NativeShow(true);
    //gtk_window_destroy(GTK_WINDOW(tmpWindow));
  }
...
LayoutDeviceIntPoint nsWindow::GdkPointToDevicePixels(const GdkRectangle& aPoint) {
  double scale = FractionalScaleFactor();
...
void nsWindow::LockNativePointer() {
  ...
  wl_surface* surface =
      gdk_wayland_surface_get_wl_surface(GetToplevelSurface());
  if (!surface) {
...
nsWindow* nsWindow::GetWindow(GdkSurface* window) {
...
void nsWindow::OnMap() {
...

  {
    ...
    //if (mIsAlert) {
    //  gdk_window_set_override_redirect(GetToplevelSurface(), TRUE);
    //}
